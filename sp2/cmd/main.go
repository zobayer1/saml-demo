...
func main() {
	...
	log.Infof("Service#2 Provider %s\n", version)
	...
	mux.HandleFunc("/metadata", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Disposition", "inline; filename=\"sp2-metadata.xml\"")
		http.ServeFile(w, r, "static/sp2-metadata.xml")
	})
	mux.HandleFunc("/health", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		_, err := w.Write([]byte(`{"status":"healthy","service":"sp2","timestamp":"` +
